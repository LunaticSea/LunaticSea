...
local command, get = require('class')('Filter:Pitch')
...
function get:description()
	return 'Sets the pitch of the song.'
end
...
function command:run(client, handler)
  ...
  player.filter:setTimescale({ pitch = value })

  local embed = {
    description = client.i18n:get(handler.language, 'command.filter', 'pitch_on', { value }),
    color = discordia.Color.fromHex(client.config.bot.EMBED_COLOR).value,
  }
