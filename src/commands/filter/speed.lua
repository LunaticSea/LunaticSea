...
local command, get = require('class')('Filter:Speed')
...
function get:description()
	return 'Sets the speed of the song.'
end
...
function command:run(client, handler)
  ...
  local embed = {
    description = client.i18n:get(handler.language, 'command.filter', 'speed_on', { value }),
    color = discordia.Color.fromHex(client.config.bot.EMBED_COLOR).value,
  }
