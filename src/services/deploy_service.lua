...
function deploy_service:register()
  ...
  local store = table.filter(self.client._commands, function (command)
    if command.usingInteraction then return true end
  end)
  ...
  self.client._logd:info('DeployService', 'Convert to body completed, now register all commands to discord')
...
function deploy_service:parseEngine(store)
  return table.reduce(store, function (all, current)
